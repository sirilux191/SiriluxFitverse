...
const useFacilityListStore = create(
  persist(
    (set, get) => ({
      facilities: [],
      availableSlots: [],
      ...
      fetchFacilities: async () => {
        try {
          ...
          console.log(result);
          set({ facilities: result, isLoading: false });
        } catch (error) {
      ...
      fetchAvailableSlots: async (idToVisit) => {
        try {
          ...
          if (Array.isArray(result)) {
            console.log("Available Slots for Facility ID:", idToVisit);
...
export default useFacilityListStore;
